...
# Load existing actions from JSON file
with open('actions.json', 'r') as f:
    data = json.load(f)
    actions = np.array(data["actions"])

# Prompt user to select an action to overwrite
print("Available actions:", actions)
selected_action = input("Enter the action you want to overwrite: ")

if selected_action not in actions:
    print(f"Action '{selected_action}' not found in available actions.")
    exit()
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    for sequence in range(no_sequences):
        for frame_num in range(sequence_length):
            ...
            if frame_num == 0:
                cv2.putText(image, f'STARTING COLLECTION for {selected_action}', (120, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 4, cv2.LINE_AA)
                cv2.putText(image, f'Collecting frames for {selected_action}, Video {sequence}', (15, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
                cv2.waitKey(1000)
            else:
                cv2.putText(image, f'Collecting frames for {selected_action}, Video {sequence}', (15, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
            
            keypoints = get_key_points(results)
            np.save(os.path.join(DATA_PATH, selected_action, str(sequence), str(frame_num)), keypoints)
